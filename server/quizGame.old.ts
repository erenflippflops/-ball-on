...
export function submitAnswer(room: QuizRoom, playerId: string, answer: unknown) { if (!room.question || room.status!=='playing') throw new Error('Aktif soru yok'); if (room.answered.some(a=>a.playerId===playerId)) throw new Error('Cevabın zaten gönderildi'); const player=room.players.find(p=>p.id===playerId); if (!player) throw new Error('Oyuncu bulunamadı'); const result=scoreAnswer(room.question, answer); const answered:QuizAnswer={playerId,answer,submittedAt:Date.now(),pointsEarned:result.points,correct:result.correct}; if (result.correct) player.streak++; else player.streak=0; const base=result.points; const place=room.answered.filter(a=>a.correct).length; const speed=place===0?.3:place===1?.2:place===2?.1:0; const streak=result.correct&&player.streak>=3?.5:0; const earned=Math.floor(base*(1+speed+streak)); answered.pointsEarned=earned; player.score+=earned; room.roundScores[playerId]=earned; room.answered.push(answered); return answered; }
